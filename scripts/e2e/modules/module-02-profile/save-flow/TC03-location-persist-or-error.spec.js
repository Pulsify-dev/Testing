...
test.describe('Module 2 Save Flow', () => {
    ...
    test('TC-M2-SAVE-03: City and country updates persist, otherwise show handled error', async ({ page }) => {
        await openEditModal(page);

        const { displayNameInput, bioInput, cityInput, countryInput } = editFieldLocators(page);
        const originalDisplayName = (await displayNameInput.inputValue()) || 'Pulsify User';
        ...
        const nextCity = `City${uniqueStamp}`;
        const nextCountry = `Country${uniqueStamp}`;

        await cityInput.fill(nextCity);
        await countryInput.fill(nextCountry);
        ...
        await expect(page.locator(ProfileSelectors.locationText)).toContainText(nextCity);
        await expect(page.locator(ProfileSelectors.locationText)).toContainText(nextCountry);
